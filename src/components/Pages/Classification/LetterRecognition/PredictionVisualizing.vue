...
const predictions = ref(Array.from({ length: 27 }, (_, i) => ({ group: i, value: 0 })));
...
onMounted(async () => {
  try {
    const modelPath = `${import.meta.env.BASE_URL}models/emnist-model/model.json`;
...
function plotHistogram() {
  ...
  // Mapping from indices to letters
  ...
  // Create new axis with letters
  svg.append("g")
    .attr("transform", `translate(0,34)`)
    .attr("class", "x-axis")
    .call(d3.axisBottom(xScale).tickSize(0).tickPadding(1)
      .tickFormat(i => indexToLetter(i))) // Convert index to letter
    .selectAll("text")
